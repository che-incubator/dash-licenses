...
describe('Yarn Package Manager Files Integration', () => {
  const yarnFiles = [
    'src/package-manager/yarn/parser.ts',
    'src/package-manager/yarn/bump-deps.ts',
    'src/package-manager/yarn3/parser.ts',
    'src/package-manager/yarn3/bump-deps.ts'
  ];

  const compiledFiles = [
    'dist/package-manager/yarn/parser.js',
    'dist/package-manager/yarn/bump-deps.js',
    'dist/package-manager/yarn3/parser.js',
    'dist/package-manager/yarn3/bump-deps.js'
  ];
  ...
  describe('Source Files', () => {
    yarnFiles.forEach((filePath) => {
      ...
      it(`should have ${filePath} file contain proper license header`, () => {
        ...
        expect(content).toContain('Copyright (c) 2020-2025 Red Hat, Inc.');
  ...
  describe('TypeScript Interfaces and Types', () => {
    it('should have yarn/parser.ts define YarnLicenseTable interface', () => {
      const content = fs.readFileSync('src/package-manager/yarn/parser.ts', 'utf8');
    ...
    it('should have yarn/bump-deps.ts import LicenseInfo from document', () => {
      const content = fs.readFileSync('src/package-manager/yarn/bump-deps.ts', 'utf8');
      expect(content).toContain('import {');
    ...
    it('should have yarn3/bump-deps.ts define proper interfaces', () => {
      const content = fs.readFileSync('src/package-manager/yarn3/bump-deps.ts', 'utf8');
  ...
  describe('Environment Variable Usage', () => {
    it('should have files use proper environment variables', () => {
      const content1 = fs.readFileSync('src/package-manager/yarn/parser.ts', 'utf8');
      expect(content1).toContain('process.env.TMP_DIR');

      const content2 = fs.readFileSync('src/package-manager/yarn/bump-deps.ts', 'utf8');
      expect(content2).toContain('process.env.ENCODING');
      expect(content2).toContain('process.env.DEPS_COPY_DIR');

      const content3 = fs.readFileSync('src/package-manager/yarn3/bump-deps.ts', 'utf8');
  ...
  describe('Command Line Arguments', () => {
    it('should have yarn3/parser handle command line arguments', () => {
      const content = fs.readFileSync('src/package-manager/yarn3/parser.ts', 'utf8');
