...
(async function main() {
  ...
  try {
    ...
    console.log(`Changing directory to ${env.PROJECT_COPY_DIR}...`);
    ...
    const parserScript = path.join(env.WORKSPACE_DIR, 'package-managers/npm/parser.js');
    ...
    const bumpDepsScript = path.join(env.WORKSPACE_DIR, 'package-managers/npm/bump-deps.js');
    let restricted = 0;
    try {
