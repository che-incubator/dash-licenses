...
(async function main() {
  ...
  try {
    ...
    const parserScript = path.join(env.WORKSPACE_DIR, 'package-managers/yarn/parser.js');
    ...
    const allDepsFile = path.join(env.TMP_DIR, 'yarn-all-deps.json');
    execSync(
    ...
    const bumpDepsScript = path.join(env.WORKSPACE_DIR, 'package-managers/yarn/bump-deps.js');
    let restricted = 0;
    try {
